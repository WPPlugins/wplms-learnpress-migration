...
class WPLMS_LEARNPRESS_INIT{
    ...
    function build_curriculum($course_id){
    	...
    	$sections_units_quizzes = $wpdb->get_results("
		    			SELECT section_id, section_description as id
						...
						ORDER BY section_id
		    		");
    	if(!empty($sections_units_quizzes)){
    		$curriculum = array();
    		foreach ($sections_units_quizzes as $section_unit_quiz) {
    			$curriculum[] = $section_unit_quiz->id;

    			if(is_numeric($section_unit_quiz->id)){
    				$check_post_type = get_post_type($section_unit_quiz->id);
    				if($check_post_type  == 'unit'){
    					$this->migrate_unit_settings($section_unit_quiz->id);
    				}
    				if($check_post_type  == 'quiz'){
    					$this->migrate_quiz_settings($section_unit_quiz->id);
    				}
    			}
    		}
